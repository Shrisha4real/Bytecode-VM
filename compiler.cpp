...
LocalCompiler::LocalCompiler(FunctionType type,
                             std::shared_ptr<LocalCompiler> enclosing)
    : local_count(1), scope_depth(0), locals(), type(type),
      function(std::make_shared<ObjFunction>()), enclosing(enclosing) {};

/*
 * Function: contructor
 * Purpose : creates a Scanner and a parser onjects and sets all the parsing
 * rules got every keyword Returns : InterpretResult - the result of program
 * execution.
 */
Compiler::Compiler(const std::string &source,
                   std::shared_ptr<StringInterner> string_table)
    : source(source), rules(static_cast<size_t>(token_type::TOKEN_EOF) + 1),
  ...
  rules[token_type::TOKEN_LEFT_PAREN] = {
      std::bind(&Compiler::grouping, this, _1),
      std::bind(&Compiler::call, this, _1), Precedence::PREC_CALL};
  rules[token_type::TOKEN_RIGHT_PAREN] = {nullptr, nullptr,
                                          Precedence::PREC_NONE};
  rules[token_type::TOKEN_LEFT_BRACE] = {nullptr, nullptr,
                                         Precedence::PREC_NONE};
  rules[token_type::TOKEN_RIGHT_BRACE] = {nullptr, nullptr,
                                          Precedence::PREC_NONE};
  ...
  rules[token_type::TOKEN_MINUS] = {std::bind(&Compiler::unary, this, _1),
                                    std::bind(&Compiler::binary, this, _1),
                                    Precedence::PREC_TERM};
  rules[token_type::TOKEN_PLUS] = {
      nullptr, std::bind(&Compiler::binary, this, _1), Precedence::PREC_TERM};
  rules[token_type::TOKEN_SEMICOLON] = {nullptr, nullptr,
                                        Precedence::PREC_NONE};
  rules[token_type::TOKEN_SLASH] = {
      nullptr, std::bind(&Compiler::binary, this, _1), Precedence::PREC_FACTOR};
  rules[token_type::TOKEN_STAR] = {
      nullptr, std::bind(&Compiler::binary, this, _1), Precedence::PREC_FACTOR};
  rules[token_type::TOKEN_BANG] = {std::bind(&Compiler::unary, this, _1),
                                   nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_BANG_EQUAL] = {nullptr,
                                         std::bind(&Compiler::binary, this, _1),
                                         Precedence::PREC_EQUALITY};
  rules[token_type::TOKEN_EQUAL] = {nullptr, nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_EQUAL_EQUAL] = {
      nullptr, std::bind(&Compiler::binary, this, _1),
      Precedence::PREC_EQUALITY};
  rules[token_type::TOKEN_GREATER] = {nullptr,
                                      std::bind(&Compiler::binary, this, _1),
                                      Precedence::PREC_EQUALITY};
  rules[token_type::TOKEN_GREATER_EQUAL] = {
      nullptr, std::bind(&Compiler::binary, this, _1),
      Precedence::PREC_EQUALITY};
  rules[token_type::TOKEN_LESS] = {nullptr,
                                   std::bind(&Compiler::binary, this, _1),
                                   Precedence::PREC_EQUALITY};
  rules[token_type::TOKEN_LESS_EQUAL] = {nullptr,
                                         std::bind(&Compiler::binary, this, _1),
                                         Precedence::PREC_EQUALITY};
  // rules[token_type::TOKEN_INCREMENT] = { nullptr,
  // std::bind(&Compiler::variable, this, _1), Precedence::PREC_POSTFIX};
  // rules[token_type::TOKEN_DECREMENT] = { nullptr,
  // std::bind(&Compiler::variable, this, _1), Precedence::PREC_POSTFIX };
  rules[token_type::TOKEN_IDENTIFIER] = {
      std::bind(&Compiler::variable, this, _1), nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_STRING] = {std::bind(&Compiler::string, this, _1),
                                     nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_NUMBER] = {std::bind(&Compiler::number, this, _1),
                                     nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_AND] = {nullptr, std::bind(&Compiler::and_, this, _1),
                                  Precedence::PREC_AND};
  ...
  rules[token_type::TOKEN_FALSE] = {std::bind(&Compiler::literal, this, _1),
                                    nullptr, Precedence::PREC_NONE};
  ...
  rules[token_type::TOKEN_NIL] = {std::bind(&Compiler::literal, this, _1),
                                  nullptr, Precedence::PREC_NONE};
  rules[token_type::TOKEN_OR] = {nullptr, std::bind(&Compiler::or_, this, _1),
                                 Precedence::PREC_OR};
  ...
  rules[token_type::TOKEN_TRUE] = {std::bind(&Compiler::literal, this, _1),
                                   nullptr, Precedence::PREC_NONE};
...
Chunk *Compiler::current_chunk() { return &current->function->chunk; }
/*
 * Function: compile
 * Purpose : compiles the given expression
 * Returns : true if the expression is valid else(or compilation is sucessful
 * else return false.
 */
std::shared_ptr<ObjFunction> Compiler::compile() {
  // init_compiler
  std::shared_ptr<LocalCompiler> compiler =
      std::make_shared<LocalCompiler>(FunctionType::TYPE_SCRIPT, current);
  current = compiler;
  compiler->enclosing = current;
  // init_compiler
  if (current->type != FunctionType::TYPE_SCRIPT) {
    current->function->name = string_table->copy_string(
        parser->previous.start, parser->previous.length);
  }
  this->parser
      ->advance(); // sets the first token as the current token to be parsed
...
void Compiler::emit_constant(Value &&value) {
  this->emit_bytes(OpCode::OP_CONSTANT,
                   Compiler::make_constant(std::move(value)));
}
 ...
 * Purpose : starts at the current token and parses any expression at the given
...
void Compiler::unary(bool can_assign) {
  ...
  case token_type::TOKEN_BANG:
    this->emit_byte(OP_NOT);
    break;
...
void Compiler::binary(bool can_assign) {
  ...
  switch (operator_type) {
  case token_type::TOKEN_BANG_EQUAL:
    this->emit_bytes(OpCode::OP_EQUAL, OpCode::OP_NOT);
    break;
  case token_type::TOKEN_EQUAL_EQUAL:
    this->emit_byte(OpCode::OP_EQUAL);
    break;
  case token_type::TOKEN_GREATER:
    this->emit_byte(OpCode::OP_GREATER);
    break;

  case token_type::TOKEN_GREATER_EQUAL:
    this->emit_bytes(OpCode::OP_LESS, OpCode::OP_NOT);
    break;
  case token_type::TOKEN_LESS:
    this->emit_byte(OpCode::OP_LESS);
    break;
  case token_type::TOKEN_LESS_EQUAL:
    this->emit_bytes(OpCode::OP_GREATER, OpCode::OP_NOT);
    break;
...
void Compiler::literal(bool can_assign) {
  ...
  case token_type::TOKEN_FALSE:
    this->emit_byte(OpCode::OP_FALSE);
    break;
...
void Compiler::grouping(bool can_assign) {
  Compiler::expression();
  this->parser->consume(token_type::TOKEN_RIGHT_PAREN,
                        "Expect ')' after expression.");
}

void Compiler::string(bool can_assign) {
  emit_constant(Value::Obj(std::shared_ptr<Object>{string_table->copy_string(
      this->parser->previous.start + 1, this->parser->previous.length - 2)}));
}

ParseRule *Compiler::get_rule(token_type type) { return &this->rules[type]; }
...
void Compiler::expression_statement() {
  expression();
  this->parser->consume(token_type::TOKEN_SEMICOLON,
                        "Expect ';' after expression.");
...
void Compiler::synchronise() {
  ...
  while (parser->current.type != token_type::TOKEN_EOF) {
    if (parser->previous.type == token_type::TOKEN_SEMICOLON)
      return;
...
void Compiler::block() {
  while (!check(token_type::TOKEN_RIGHT_BRACE) &&
...
int Compiler::resolve_local(std::shared_ptr<LocalCompiler> compiler,
                            Token *name) {
...
void Compiler::for_statement() {
  ...
  if (!match(token_type::TOKEN_RIGHT_PAREN)) {
    ...
    expression();
    emit_byte(OP_POP);
    parser->consume(token_type::TOKEN_RIGHT_PAREN,
...
void Compiler::emit_loop(int loop_start) {
  ...
  if (offset > UINT16_MAX)
    parser->error("Loop body too largre");
...
void Compiler::function(FunctionType type) {
  // init_compiler
  std::shared_ptr<LocalCompiler> compiler =
      std::make_shared<LocalCompiler>(type, current);
  compiler->enclosing = current;
  current = compiler;
  ...
  if (compiler->type != FunctionType::TYPE_SCRIPT) {
    compiler->function->name = string_table->copy_string(
        parser->previous.start, parser->previous.length);
  }
  begin_scope();
  parser->consume(token_type::TOKEN_LEFT_PAREN,
  ...
  parser->consume(token_type::TOKEN_LEFT_BRACE,
                  "Expect '{' before function body.");
  block();
  // std::shared_ptr<LocalCompiler> end =
  // std::make_shared<LocalCompiler>(compiler);
  std::shared_ptr<ObjFunction> function = end_compiler();
  emit_bytes(OpCode::OP_CONSTANT,
             make_constant(Value::Obj(std::move(function))));
}
...
void Compiler::return_statement() {
  ...
  if (match(token_type::TOKEN_SEMICOLON)) {
    ...
    expression();
    parser->consume(token_type::TOKEN_SEMICOLON,
                    "Expect a ; after return value");
...
void Compiler::parse_clean_statement() {
  ...
  this->parser->consume(token_type::TOKEN_IDENTIFIER,
                        "Expect declared variable");
  this->parser->consume(TOKEN_SEMICOLON,
...
void Compiler::parse_train_statement(bool can_assign) {
  // 1. Consume model name
  // consume_model(TOKEN_MODEL, "Expect model name after 'train'.");
  // uint8_t model_var = identifier_constant(this->parser->previous); // e.g.,
  // "RandomForest" this->parser->consume(TOKEN_IDENTIFIER, "Expect model name
  // after 'train'.");

  uint8_t model_var = parse_variable(
      "Expect model name after 'train'."); // e.g., "RandomForest"
  ...
  if (match(TOKEN_LEFT_PAREN)) {
    ...
    expression();
    has_param = true;
    this->parser->consume(TOKEN_RIGHT_PAREN,
                          "Expect ')' after model parameters.");
  }
